...
const viewport = document.getElementById('brainViewport');
const exchangeLog = document.getElementById('exchangeLog');
document.getElementById('openEcosystemBtn').addEventListener('click', () => api.showWindow('ecosystem'));
document.getElementById('openTrainingBtn').addEventListener('click', () => api.showWindow('training'));
document.getElementById('openTrainingCard').addEventListener('click', () => api.showWindow('training'));
...
let dragX = 0, dragY = 0;
let labelTimer = null;
let outgoing = null;       // strongest measured outgoing edges for each simulated neuron
const exchangePool = [];
const exchangeState = [];
let exchangeNext = 0;
let lastExchangeLogAt = 0;
...
function build(points, circuit) {
  ...
  // Keep the strongest outgoing measured connections for every neuron. When a
  // source neuron spikes, LIFSim propagates along all of its outgoing edges;
  // the renderer samples these strongest edges so the moving lines represent
  // real graph paths without trying to draw ~19k connections every frame.
  outgoing = Array.from({ length: n }, () => []);
  for (const edge of circuit.edges) {
    const [pre, post, weight] = edge;
    if (pre >= 0 && pre < n && post >= 0 && post < n) outgoing[pre].push({ post, weight });
  }
  ...
  // Reusable line objects avoid allocating WebGL resources during a spike
  // burst. Green/cyan lines are excitatory and red lines are inhibitory.
  for (let i = 0; i < 72; i++) {
    const geometry = new THREE.BufferGeometry();
    geometry.setAttribute('position', new THREE.Float32BufferAttribute(new Float32Array(6), 3));
    const material = new THREE.LineBasicMaterial({ color: 0x77e7a0, transparent: true,
      opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false });
    const line = new THREE.Line(geometry, material);
    line.visible = false;
    group.add(line);
    exchangePool.push(line);
    exchangeState.push({ ttl: 0, duration: 0.42 });
  }
...
const ROLE_NAMES = {
  lc4: 'LC4 逼近', lplc2: 'LPLC2 逼近', gf: 'DNp01 逃生',
  dna01: 'DNa01 转向', dna02: 'DNa02 转向', mdn: 'MDN 后退',
  dnp09: 'DNp09 前进', dng11: 'DNg11 梳理', escw: '逃生翼', other: '中间神经元',
};

function roleName(index) {
  const role = neurons?.roles[index] || 'other';
  const type = neurons?.types[index];
  return ROLE_NAMES[role] || (type && type !== '?' ? type : role);
}

// Visualize one actual source→target edge from the loaded circuit. This is a
// propagation sample, not a claim that the line is an imaged biological axon.
function showExchange(source, edge) {
  if (!neurons || !edge) return;
  const lineIndex = exchangeNext;
  exchangeNext = (exchangeNext + 1) % exchangePool.length;
  const line = exchangePool[lineIndex];
  const values = line.geometry.attributes.position.array;
  const p = neurons.positions;
  values[0] = p[3 * source]; values[1] = p[3 * source + 1]; values[2] = p[3 * source + 2];
  values[3] = p[3 * edge.post]; values[4] = p[3 * edge.post + 1]; values[5] = p[3 * edge.post + 2];
  line.geometry.attributes.position.needsUpdate = true;
  line.geometry.computeBoundingSphere();
  line.material.color.setHex(edge.weight >= 0 ? 0x77e7a0 : 0xff6b72);
  line.material.opacity = 0.92;
  line.visible = true;
  exchangeState[lineIndex] = { ttl: 0.42, duration: 0.42 };

  // The text log is throttled so high-rate spikes cannot overload the DOM.
  const now = performance.now();
  if (now - lastExchangeLogAt < 85) return;
  lastExchangeLogAt = now;
  if (exchangeLog.firstElementChild?.textContent?.startsWith('等待')) exchangeLog.textContent = '';
  const item = document.createElement('li');
  item.className = edge.weight >= 0 ? 'exc' : 'inh';
  item.textContent = `${(now / 1000).toFixed(2)}s  ${roleName(source)} → ${roleName(edge.post)}  w=${Number(edge.weight).toFixed(2)}`;
  exchangeLog.prepend(item);
  while (exchangeLog.children.length > 8) exchangeLog.lastElementChild.remove();
}
...
function regionName(picked) {
  ...
  const sideSuffix = (role) => {
    ...
    return l === r ? '' : (l > r ? ' · 左侧' : ' · 右侧');
  };
  switch (major) {
    case 'lc4': case 'lplc2': return `⚡ 逼近检测神经元 (LC4/LPLC2)${sideSuffix(major)}`;
    case 'gf': return '⚡ 巨纤维 (DNp01) — 触发逃生';
    case 'dna01': case 'dna02': return `⚡ 转向神经元 (DNa01/02)${sideSuffix(major)}`;
    case 'dnp09': return '⚡ 行走指令神经元 (DNp09)';
    case 'dng11': return '⚡ 梳理指令神经元 (DNg11)';
    case 'escw': return '⚡ 逃生翼下行神经元 (DNp02/04/11)';
    case 'mdn': return '⚡ 后退神经元 (MDN)';
    default: {
...
// The dashboard occupies part of the window, so the WebGL viewport must follow
// the actual left-hand pane rather than the outer BrowserWindow size.
...
api.onSpikes((list) => {
  for (let i = 0; i < list.length; i++) {
    const event = list[i];
    flash(event.neuron, event.isGF);

    // Draw a bounded sample of propagation paths. Alternating among the four
    // strongest outgoing edges avoids implying that only one synapse exists.
    if (i < 12 && outgoing?.[event.neuron]?.length) {
      const edges = outgoing[event.neuron];
      showExchange(event.neuron, edges[(exchangeNext + i) % edges.length]);
    }
  }
});
...
const RATE_KEYS = ['population', 'looming', 'steerLeft', 'steerRight',
  'forward', 'backward', 'grooming', 'escapeWing'];
let firstTelemetrySeen = false;
const BEHAVIOR_NAMES = {
  idle: '静止', walking: '行走', flying: '飞行', grooming: '梳理',
  feeding: '进食', escaping: '逃生', dead: '已死亡', startle: '惊跳',
};

function setRate(key, rawValue) {
  const value = Number.isFinite(Number(rawValue)) ? Math.max(0, Number(rawValue)) : 0;
  document.getElementById(`rate-${key}`).textContent = value.toFixed(1);
  // 200 Hz is only a display ceiling; the un-clamped number remains visible.
  document.getElementById(`bar-${key}`).style.width = `${Math.min(100, value / 2)}%`;
}
...
api.onNeuralTelemetry((snapshot) => {
  if (!snapshot) return;
  // A one-time ready marker is useful in DESKTOPFLY_DEBUG mode and stays quiet
  // during normal use. main.js only forwards info-level renderer logs in that
  // explicit debug mode.
  if (!firstTelemetrySeen) {
    firstTelemetrySeen = true;
    console.info(`Neural telemetry connected at simulation ${snapshot.simMs || 0} ms`);
  }
  document.getElementById('simTime').textContent = `${(Number(snapshot.simMs || 0) / 1000).toFixed(1)} s`;
  document.getElementById('totalSpikes').textContent = Number(snapshot.totalSpikes || 0).toLocaleString('zh-CN');
  document.getElementById('brainBehavior').textContent = BEHAVIOR_NAMES[snapshot.behavior] || snapshot.behavior || '未知';
  for (const key of RATE_KEYS) setRate(key, snapshot.rates?.[key]);
  document.getElementById('inputLoomL').textContent = asPercent(snapshot.inputs?.loomLeft);
  document.getElementById('inputLoomR').textContent = asPercent(snapshot.inputs?.loomRight);
  document.getElementById('inputPuff').textContent = asPercent(snapshot.inputs?.airPuff);
  document.getElementById('inputGait').textContent = asPercent(snapshot.inputs?.gait);
  if (snapshot.trainedSkill?.name) {
    document.getElementById('activeSkill').textContent = `已启用：${snapshot.trainedSkill.name}`;
  }
});
...
function frame(tMs) {
  ...
  for (let i = 0; i < exchangePool.length; i++) {
    const state = exchangeState[i];
    if (state.ttl <= 0) continue;
    state.ttl -= dt;
    if (state.ttl <= 0) { exchangePool[i].visible = false; continue; }
    exchangePool[i].material.opacity = 0.92 * (state.ttl / state.duration);
  }
  if (stimRingT > 0) {
    stimRingT -= dt;
...
(async () => {
  ...
  console.info(`Neural lab ready: ${data.points.points.length} anatomy points, ` +
    `${data.circuit.neurons.length} simulated neurons, ${data.circuit.edges.length} circuit edges`);
  requestAnimationFrame(frame);
})();
