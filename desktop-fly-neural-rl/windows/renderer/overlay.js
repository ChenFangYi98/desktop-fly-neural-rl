...
let trainedSkill = null;
let trainedSkillCooldown = 0;
...
// A learned skill is an engineered policy layer, not a rewritten connectome.
// It watches one documented sensory/body condition and, when that condition is
// active, injects the learned action into the corresponding identified neural
// population. The normal LIF graph still determines the downstream response.
function applyTrainedSkill(dt, fly) {
  trainedSkillCooldown = Math.max(0, trainedSkillCooldown - dt);
  if (!trainedSkill || !sim || !fly || trainedSkillCooldown > 0) return;
  const trigger = trainedSkill.trigger;
  const active = (trigger === 'loomLeft' && sim.loomL > 0.16 && sim.loomL > sim.loomR * 1.15)
    || (trigger === 'loomRight' && sim.loomR > 0.16 && sim.loomR > sim.loomL * 1.15)
    || (trigger === 'loomCenter' && Math.min(sim.loomL, sim.loomR) > 0.13)
    || (trigger === 'airPuff' && sim.airPuff > 0.18)
    || (trigger === 'idle' && fly.state === 'idle');
  if (!active) return;
  stimulateGroup(trainedSkill.action);
  trainedSkillCooldown = 0.7;
}
...
function tick(dt) {
  ...
  if (sim && first) {
    ...
    // Send a compact 10 Hz snapshot to the neural lab. These values are read
    // directly from the running LIF simulation and SignalBuilder; the brain
    // renderer never fabricates neural rates.
    telemetryAccumulator += dt;
    if (telemetryAccumulator >= 0.10) {
      telemetryAccumulator %= 0.10;
      api.sendNeuralTelemetry({
        simMs: sim.simMs,
        totalSpikes: sim.totalSpikes,
        behavior: first.state,
        rates: {
          population: sim.ratePop,
          looming: sim.rateLoom,
          steerLeft: sim.rateDNaL,
          steerRight: sim.rateDNaR,
          backward: sim.rateMDN,
          forward: sim.rateFwd,
          grooming: sim.rateGroom,
          escapeWing: sim.rateEscW,
        },
        inputs: {
          loomLeft: sim.loomL,
          loomRight: sim.loomR,
        ...
        outputs: {
          ...
          walk: signals.walkDrive,
          turn: signals.turnBias,
          groom: signals.groomDrive,
          nervous: signals.nervous,
        },
        trainedSkill: trainedSkill ? { name: trainedSkill.name, trigger: trainedSkill.trigger,
          action: trainedSkill.action, confidence: trainedSkill.confidence } : null,
      });
    }
...
api.onTrainedSkill((skill) => {
  trainedSkill = skill && skill.trigger && skill.action ? skill : null;
  trainedSkillCooldown = 0;
});
