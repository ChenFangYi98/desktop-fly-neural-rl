...
export class LIFSim {
  constructor(circuit, spikeBus = null, locomotorCircuit = null) {
    ...
    // Learning never rewires the connectome.  A bounded multiplicative gain
    // sits beside every measured FlyWire edge so exported plasticity can be
    // mapped back onto the exact signed topology at deployment time.
    this.edgeGain = new Float32Array(edges.length);
    this.edgeGain.fill(1);
    this.edgeSource = new Int32Array(edges.length);
  ...
  setEdgeGains(gains) {
    if (!gains || gains.length !== this.edgeGain.length) {
      throw new Error(`Expected ${this.edgeGain.length} connectome edge gains`);
    }
    for (let i = 0; i < gains.length; i++) {
      const value = Number(gains[i]);
      this.edgeGain[i] = Number.isFinite(value) ? Math.max(0.1, Math.min(6, value)) : 1;
    }
  }

  plasticitySummary() {
    let changed = 0, totalDelta = 0, maxDelta = 0;
    for (const gain of this.edgeGain) {
      const delta = Math.abs(gain - 1);
      if (delta > 1e-5) changed++;
      totalDelta += delta;
      maxDelta = Math.max(maxDelta, delta);
    }
    return { edgeCount: this.edgeGain.length, changed,
      meanAbsoluteGainChange: totalDelta / Math.max(1, this.edgeGain.length), maxGainChange: maxDelta };
  }
    ...
    for (let t = 0; t < ms; t++) {
      ...
      for (let s = 0; s < nSpiked; s++) {
        ...
        for (let k = this.rowStart[i]; k < end; k++) {
          const j = this.colIdx[k], wk = this.w[k] * this.edgeGain[k];
